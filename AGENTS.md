...
- Hemisphere (N/S) is derived from the selected timezone via a hardcoded `SOUTHERN_TIMEZONES` set in `src/lib/timezones.ts` (a test checks it stays a subset of the selector list). Unlisted (browser-detected) zones are placed by the direction of their DST, else by `SOUTHERN_NO_DST_TIMEZONES`, else north.
- Compare zone names only via `canonicalZone`/`listedZoneFor`: Chrome reports legacy names (`Asia/Calcutta`, `Europe/Kiev`) for listed zones. Never work out DST by hand; pass the zone name to `Intl`.
